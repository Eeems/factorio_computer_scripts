...
-- load utf8 library
local utf8_ok, utf8lib = os.pcall(require, "compat53.utf8")
if utf8_ok then
...
-- load table library
local table_ok, tablib = os.pcall(require, "compat53.table")
if table_ok then
...
local str_ok, strlib = os.pcall(require, "compat53.string")
if str_ok then
...
if not str_ok then
   local struct_ok, struct = os.pcall(require, "struct")
   if struct_ok then
...
function M.coroutine.create(func)
   local success, result = os.pcall(coroutine_create, func)
   if not success then
